...
// XC4630_images(x,y,const byte img[],s) draw image img scaled by s at x,y (see sample for img for format)
...
void XC4630_imaget2(int x,int y,const byte img[],unsigned int t){        //draws with colour t transparent, optimized to reduce areasets- heaps faster, and handles clipping at RHS better
  int w,h,u,v;
...
void XC4630_imaget(int x,int y,const byte img[],unsigned int t){        //draws with colour t transparent
  int w,h,offset,u,v;
  unsigned int c;
  w=pgm_read_byte(&img[0]);         //assume no image is bigger than 255 pixels
  h=pgm_read_byte(&img[1]);         //assume no image is bigger than 255 pixels
  for(int v=0;v<h;v++){    
    offset=v*w*2+2;
    for(int u=0;u<w;u++){
      c=pgm_read_byte(&img[u*2+offset])<<8;
      c=c|pgm_read_byte(&img[u*2+offset+1]);
      if(c!=t){XC4630_point(u+x,v+y,c);}
    }
  }
}
